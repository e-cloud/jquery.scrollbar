require.config({
    paths: {
        'jquery': '../includes/jquery',
        'google-code-prettify': '../includes/prettify/prettify',
        'jquery.scrollbar': '../jquery.scrollbar'
    }
})

require(['jquery', 'google-code-prettify', 'jquery.scrollbar'], function (jQuery, prettify) {
    jQuery(function ($) {
        ...
        $('.container').each(function () {
            ...
            console.log(1)
            $('<pre></pre>').addClass('prettyprint linenums lang-js').text(getAlignedText(content.find('pre').html())).appendTo(content.find('.js'));
